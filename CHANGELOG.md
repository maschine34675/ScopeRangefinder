...
- The showcase preset (and with it the defaults of a fresh install and the reset button) now has the zeroing line on and the ballistics line set to `Hold` in minutes of angle — most reticles in the game are MOA-graduated, so that is the unit the hold is most often read against. Existing configs keep their values.
- The four trajectory-preview fine-tuning options (near/far colors, dispersion ring and its color) are now advanced settings, shown only with the settings menu's advanced toggle. The preview itself stays where it was; the Auto Zero section is just shorter for everyone who never recolors it.
- The `Hold` row now shows its unit in milliradians too: `HLD +1.2mil` instead of a bare `+1.2`. Turret markings leave it off, but a readout that is new to someone has to answer "+0.3 of what?", and two suffixed units next to one bare one read like a bug rather than a convention. `mil` is the shooting-sports short form of the milliradian, three letters like `moa`. The compact `Dial` residual stays unit-less on purpose; its unit follows the hold unit.
- The `Dial` ballistics row is a lot narrower. It used to repeat the unit suffix the zeroing row right above already states and to print the residual in full hold format (`DIAL 0350m +1.2`), which made it 60 % wider than every other row — and the background plate grew by a fixed worst-case factor on top. Now the prefix is the three-letter `DIA` (so the prefix column is as narrow as with `RNG`/`ZRO`/`HLD`), the stop comes without the suffix, and the residual in a compact signed form (`DIA 0350+1.2`; one decimal in mil and MOA, whole centimeters), shown only when it exceeds 0.15 mil; and the plate's width now follows the ballistics row's real width instead of a fixed factor: it grows only by the ratio that row exceeds the range/zero rows of the same configuration. Plates without a ballistics row are untouched, whatever unit, prefix, or suffix they use. When no stop can be recommended — auto zero is driving the zero, or the sight has no usable stop list — the row now keeps its `DIA` prefix and shows the plain hold, instead of silently turning into an `HLD` row. One side effect is a fix in its own right: a `Hold` row in MOA or centimeters (`HLD -42.3moa`, `HLD -1845cm`) used to overhang the plate, because the old flat factor never accounted for it — it now gets the room it needs.

### Added

- Layouts can anchor the readout. Until now the offsets always positioned the block's center, so a readout parked in a corner walked off the edge whenever a row was added (zeroing line, ballistics line) and had to be re-offset. The layout editor's new `Anchor` grid picks which point the offsets pin — bottom-left for a bottom-left readout — and the block then grows away from that point: taller upward, wider rightward, never off the edge, whatever the row count, unit, or ballistics mode. Switching the anchor keeps the readout where it is on screen (the offset is converted), so the choice is only about future growth. Stored as an optional `Anchor` field in `ScopeRangefinder.layouts.json`; `Center` stays the default, so every existing layout renders exactly as before.
- Measurement robustness, as a new `Measurement` settings section, all off by default: `HoldTime` keeps the last valid distance through a short dropout instead of flashing the no-target text; `ScanMode` `Near`/`Far` holds the nearest or farthest valid target seen within `ScanWindow`, like the NEAR/FAR modes of real rangefinders, so the reticle only has to brush a target; and `FreezeHotkey` holds one reading until pressed again or the sight is lowered — a single-shot measurement that stays put while you shift to the target, marked with `°` behind the distance (one of the few glyphs every bundled font has) and never touching the zeroing. All three act on the whole measurement, so the ballistics line, a continuous auto zero, and the distance other mods read benefit the same way; a continuous zero in particular stops snapping back on every dropout.
...
- The mod now ships the 14 external font files used by its presets, together with their required license and archive-information files: Digit Tech and Home Video under CC0 1.0; DSEG, LCD14, B612 Mono, VCR OSD Replayed, Oxanium, Quantico, Rajdhani, Share Tech Mono, and VT323 under OFL 1.1. Exact upstream versions and checksums are included in `fonts/FONT-SOURCES.md`.
...
- Added style presets: named looks covering all Readout, Scope Text, and Scope Background settings, selectable from a `Style Preset` dropdown in the General settings. One click applies a preset; the current look can be saved under a new name, and own presets can be deleted from the dropdown (guarded by a confirming second click). The file layout mirrors the scope layouts: shipped presets live in a `Styles` section of the read-only `ScopeRangefinder.presets.json` — replaced wholesale on updates, so never edit it — while every preset you save lands in `ScopeRangefinder.styles.json`, which updates never touch (shipped names are reserved). Shipped presets: `RAPTAR EFT Style`, `RAPTAR Lite ES`, `RAPTAR S`, `LED Display Coral Red`, `VCR Chromatic`, `DSEG7 Mini RGB Split`, `DSEG14 Classic Amber`, `DT16 Cyrillic`, `LCD14 Starburst Red`, `B612 Cockpit Phosphor`, `Quantico Tactical Amber`, `Oxanium HUD Cyan`, `Rajdhani Tech Chartreuse`, `Terminal Green`, and `Tech Mono Ice`.
